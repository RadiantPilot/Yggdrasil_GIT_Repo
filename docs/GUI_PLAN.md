...
Fremtidige faser vil legge til:
```
    ├── widgets/                 # gjenbrukbare widgets (pid_card, pose_sliders, etc.)
    ├── models/                  # Qt table-modeller
    ├── style/                   # tema (QSS, fargepaletter)
    └── utils/                   # ring_buffer, formatting
```
...
3. `bridge/controller_bridge.py` — ekte + mock-modus
...
- Skal GUI kunne kjøre uten hardware (simulert modus)? ✅ Ja — `--mock` flag.
